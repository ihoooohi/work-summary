...
- **问题**：渲染授权检查将同一会话所有请求的消耗汇总，导致同级请求已用额度被错误计入当前请求而遭拒。
- **修复**：将视频/图片消耗读取范围缩小到当前调度任务及其子扣费记录，无请求场景保留会话级语义。
- **成果**：372 项测试通过，请求级消耗授权正确，生产会话的错误拒绝问题得到修复。

### [#7397](https://github.com/Vispie-AI/VisPie_backend/pull/7397) fix(reelcraft): honor direct video reference choice
- **日期**：2026-09-02 | **状态**：✅ 已合并
- **问题**：用户选择"不用作参考"时，分镜合成帧仍被送入视频生成请求，导致提供商以内容策略拒绝请求。
- **修复**：在故事板流水线中排除直接视频模式下的现有合成帧，保留角色/场景参考和显式帧行为。
- **成果**：86 项回归测试全部通过，BytePlus 敏感内容检测错误不再出现。

### [#7396](https://github.com/Vispie-AI/VisPie_backend/pull/7396) fix(reelcraft): restore reconnected story routes
- **日期**：2026-09-02 | **状态**：✅ 已合并
- **问题**：Story Tree 中手动连接分支后续保存时，过期的终端元数据未被清除，导致路由编译错误。
- **修复**：在原子编译前同时修复持久化 Canvas 数据和旧版手动树的终端字段，保留真实零出口和命名结局。
- **成果**：235 项后端测试通过，故事路由编译正确，零出口分支和命名结局保留正常。
